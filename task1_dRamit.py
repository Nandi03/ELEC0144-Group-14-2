...
for epoch in range(epochs):
    ...
    for i in range(len(x_train)):
        # Forward pass
        l1_output = sigmoid(np.dot(x_train[i], l1_weights) + l1_bias)  # Layer 1 with sigmoid activation
        ...
        l1_output_grad = np.dot(output_grad, l2_weights.T) * sigmoid_derivative(l1_output)
...
for i in range(len(x_test)):
    l1_input = sigmoid(np.dot(x_test[i], l1_weights) + l1_bias)
    output = np.sum(np.dot(l1_input, l2_weights) + l2_bias)
...
plt.xlabel('X_test')
plt.ylabel('Y')
plt.title('Cubic Function Approximation using Neural Network with Sigmoid Activation (Sequential Learning)')
plt.legend()
plt.grid(True)
plt.show()
