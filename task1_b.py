...
model = Model(learning_rate=0.1, optimizer="sgd", epochs=1000)
...
num_elements = int(0.8 * len(x_train))  # can change 0.8 to another value
...
# showing the x_test values on the x-axis and class on the y-axis
...
plt.ylabel('outputs')
plt.title('Cubic regression for 1-3-1 tanh sgd NN')
plt.legend()
plt.grid(True)
plt.show()
